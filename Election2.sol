...
contract Election {
    ...
    struct Candidates {
        ...
        bool criminalrecord;//stores a list of criminal records
        uint VoterID;
        bool accepted;
        bytes32 party;
    }
...
//        bytes32 nic;
        ...
        */
        for (uint i = 0; i < CandidatesNames.length; i++) {
            ...
            candidates.push(Candidates({
                ...
                voteCount: 0,
                criminalrecord = criminalRecord[i],//true if person has a criminal record
                /*see if it can be modified into a list containing all criminal records*/
    ...
    /// voteFor your vote to the voter `to`.
    function voteFor(address to) public {
        // assigns reference
        Voter storage sender = voters[msg.sender];
        require(!sender.voted, "You already voted."); // if voted

        require(to != msg.sender, "Self-delegation is disallowed.");

        /* Forward the delegation as long as
         `to` also voteFord.In general, such loops are very dangerous,
         because if they run too long, they might
         need more gas than is available in a block.
         In this case, the delegation will not be executed,
         but in other situations, such loops might cause a contract to get "stuck" completely.*/
        while (voters[to].voteFor != address(0)) {
            to = voters[to].voteFor;
        ...
        sender.voted = true;
        sender.voteFor = to;
        Voter storage voteFor_ = voters[to];
        if (voteFor_.voted) {
            // If the voteFor already voted, directly add to the number of votes
            candidates[voteFor_.vote].voteCount += sender.weight;
        } 
        else 
        {
            // If the voteFor did not vote yet, add to her weight.
            voteFor_.weight += sender.weight;
        }
    }

    /* Give your vote (including votes voteFord to you)
